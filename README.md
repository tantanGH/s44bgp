...
- XM6 Type G + X68030 (25MHz) + TS-6BE16ハイメモリ16MB + TS16DRVp.X + PCM8PP.X + Mercury-UNIT V3.5 (エミュレータ)
- XM6 Type G + X68030 (25MHz) + 060turbo互換ハイメモリ256MB + TS16DRVp.X + PCM8PP.X + Mercury-UNIT V3.5 (エミュレータ)
...
PCM8PP.X および ハイメモリドライバが有効になっていることを確認してください。(有効になっていない場合はS44BGP.X起動時にエラーメッセージが出ます)
...
S44BGP.X は OPMタイマ割り込みを使用します。FM音源ドライバーと同時に常駐しないようにしてください。
なお、バージョン 0.4.0未満は Timer-D割り込みを使用していました。
...
テキストエディタでの文書編集などはokですが、既存のファイルビュワーでのスムーススクロールはPCM8PPが停止してしまうことがあります(後述)。

MPU主体のコンパイルやA44エンコード、MP3デコード、画像ビュワーの利用などはokです。

ディスクアクセスを多用するとDMACアクセスが競合してプチノイズが乗る場合があります。RS232Cはおそらく取りこぼします。

何らかの異常状態になった場合、最悪大音量でノイズが再生される可能性があります。ヘッドフォンの利用時は特にご注意ください。

---

### 既知の問題

高速ビュワー、ファイラ内蔵ビュワーなどの一部のアプリケーションで特定の操作(特にスムーススクロールの類)を行った際にPCM8PPの再生が停止されてしまう場合があります。

こうしたアプリケーションは低クロック機でも高速で滑らかなテキストスクロールを実現するために、割り込みをブロックしてHSYNCを見ながらCRTCを直接操作してラスタコピー...といった実装になっているものが多く、PCM8PPは処理が妨げられて追いつかないと判断した時に安全をとって自動停止しているのだと思います。

この場合 S44BGP.Xは外部要因でPCM8PPが停止されたと判断し、安全のためPCM8PPをS44BGP.Xからも停止させ、"ABORTED." のメッセージを出力します。

CTRL+XF5 を押すことで次の曲の先頭から再開できます。

なお、拙作のテキストファイルビュワー TXVIEW.X は PCM8PP.X / S44BGP.X の演奏を妨げないような設計と実装になっています。そこまでの高速性は求めないけど安心して PCM8PP.X / S44BGP.X を使いたいという場合はこちらをお勧めします。

[TXVIEW.X](https://github.com/tantanGH/txview)
...
* 0.4.0 (2023/03/23) ... Timer-Dの代わりにOPMタイマを使うようにした
* 0.3.3 (2023/03/22) ... インダイレクトファイルでボリューム指定できるようにした 想定演奏時間の計算修正
