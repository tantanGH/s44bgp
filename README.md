...
テキストエディタでの文書編集などはokですが、ファイルビュワーでのスムーススクロールはPCM8PPが停止してしまうことがあります(後述)。
...
おそらく割り込みが競合してPCM8PPの処理が追いつかないと判断した時に安全をとってPCM8PPが自動停止しているのだと思います。
...
* 0.4.1 (2023/03/24) ... Abort時にPCM8PPをpauseしないようにした
